...
local function Initialize(cq, ws)
    ...
    assert(cq:wrap(Receive, ws))
    assert(cq:wrap(StdioInput))
    assert(cq:wrap(DebugInput))
    assert(cq:wrap(StatusUpdate, ws, conf.status_period))
...
local function Begin()
    ...
    if init_ok then
        ...
        LogError(-1, "Failed to initialize the sub routines for the application.")
    end
end
